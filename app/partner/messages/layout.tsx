...
export default function PartnerMessagesLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  const pathname = usePathname();
  const isDetailPage = pathname !== "/partner/messages";
  ...
  return (
    <div className="h-[calc(100vh-4rem)] bg-background flex overflow-hidden">
      ...
      <div className="hidden md:flex flex-1 w-full h-full overflow-hidden">
        {/* Left sidebar - Conversation list always visible */}
        <div className="w-[380px] border-r flex flex-col bg-background flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto py-2">
            <PartnerConversationListContent isDesktop={true} />
          </div>
        </div>
  ...
  return (
    <div className="h-full flex items-center justify-center text-center p-8 bg-white">
      <div>
        ...
        <h3 className="text-xl font-bold mb-2 text-foreground">Guest Messages</h3>
        <p className="text-sm text-muted-foreground max-w-sm leading-relaxed">
          Select a conversation to view chat history and reply to your guests
        </p>
      </div>
    </div>
  );
